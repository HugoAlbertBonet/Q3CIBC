...
class TwoChoiceDataset(Dataset):
    """Synthetic dataset for the TwoChoice-v0 single-step bimodal toy.

    Every sample is IID: draw a fresh (mode_a, mode_b) state, then a fresh
    50/50 coin flip picks which mode the expert action equals. No episode
    structure (max_episode_steps=1), so frame_stack must stay 1 and there's
    no stack_frames/episode_starts bookkeeping to get right — this is the
    simplest dataset in the repo.

    State: [mode_a, mode_b] in [-1, 1], circularly separated by >=
        min_separation (matches TwoChoiceEnv.reset exactly, so the dataset
        distribution matches the eval env's).
    Action: scalar in [-1, 1], equal to mode_a or mode_b exactly (the
        "expert" — no noise, since ANY deviation would make the argmax
        target ambiguous about which mode was intended).
    """
    ...
    def __init__(
        ...
        if frame_stack != 1:
            raise ValueError("TwoChoiceDataset has no episode structure; frame_stack must be 1")
        self.frame_stack = frame_stack
        self.min_separation = min_separation

        rng = np.random.default_rng(seed=42)

        mode_a = rng.uniform(-1.0, 1.0, size=size).astype(np.float32)
        mode_b = rng.uniform(-1.0, 1.0, size=size).astype(np.float32)
        diff = np.abs(mode_a - mode_b)
        circ_dist = np.minimum(diff, 2.0 - diff)
        need_resample = circ_dist < min_separation
        while need_resample.any():
            n = int(need_resample.sum())
            mode_b[need_resample] = rng.uniform(-1.0, 1.0, size=n).astype(np.float32)
            diff = np.abs(mode_a - mode_b)
            circ_dist = np.minimum(diff, 2.0 - diff)
            need_resample = circ_dist < min_separation

        side = rng.random(size) < 0.5
        actions = np.where(side, mode_a, mode_b).astype(np.float32)

        self.observations = np.stack([mode_a, mode_b], axis=1)
...
class PointMazePillarDataset(Dataset):
    ...
    the real MuJoCo env (simulations.point_maze_pillar_env), not a hand-coded

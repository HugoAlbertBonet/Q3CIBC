#!/usr/bin/env python3
"""Deploy a trained IBC Push-T policy on the real WidowX arm.

This is the IBC twin of ``scripts/deploy_pusht_real.py``: same CLI, same robot
plumbing, same observation pipeline, same safety guards, same ``--measure``
scoring into the same results table. **Only the policy differs** -- Q3C's
control-point generator + Q ranker is replaced by IBC's single energy model
(PixelEBM) searched with derivative-free optimization. Keep the two files in
step; a divergence anywhere but the policy block makes the head-to-head
comparison on this rig meaningless.
...
IBC-specific (differs from the BFN reference by necessity):
...
Usage (server already up):
    ...
    python scripts/deploy_pusht_real_ibc.py \
        --seed-dir checkpoints/pusht_real_ibc/seed_0029 \
        --device cpu --dry-run
    python scripts/deploy_pusht_real_ibc.py \
        --seed-dir checkpoints/pusht_real_ibc/seed_0029 \
...
(``ibc``, the one this script deploys), the checkpoint, the inference settings,
...
interrupted, and a failure there never fails the run::
    ...
    python scripts/deploy_pusht_real_ibc.py \
        --seed-dir checkpoints/pusht_real_ibc/seed_0029 \
        --steps 200 --inference dfo --refine-iters 3 \
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--ckpt-step", type=int, default=-1,
                   help="load q_estimator_step{N:06d}.pt instead of the final "
                        "q_estimator.pt (for a seed still training). IBC saves "
                        "no EMA copy, so there is no --no-ema here.")
    ...
    p.add_argument("--move-to-demo-start", dest="move_to_demo_start",
                   ...
                   help="after reset, move the EEF to the demo start pose in "
                        "--start-eep-npy (same as the q3c deploy).")
    ...
    p.add_argument("--dfo-samples", type=int, default=None,
                   help="size of the uniform action cloud IBC searches. Default: "
                        "the checkpoint's own setting (pixel_ebm_best.gin uses "
                        "2048). Applies to every --inference mode, since all of "
                        "them start from this cloud -- IBC has no proposal "
                        "network to draw candidates from.")
    p.add_argument("--inference", choices=["argmax", "sample", "langevin", "dfo"],
                   default="dfo",
                   help="action selection. dfo (default) = IBC's iterative_dfo, "
                        "i.e. what the checkpoint was trained to be served with "
                        "(score -> softmax resample -> shrinking Gaussian jitter). "
                        "argmax/sample = rank the initial uniform cloud once, with "
                        "no refinement (diagnostic: it shows the raw energy "
                        "surface). langevin = refine the cloud with Langevin MCMC "
                        "instead, the gradient-based sampler from the same IBC "
                        "paper.")
    p.add_argument("--refine-iters", type=int, default=None,
                   help="langevin/dfo refinement iterations. Default: the "
                        "checkpoint's own setting (pixel_ebm_best.gin uses 3 for "
                        "DFO); 50 is the usual Langevin count.")
    p.add_argument("--langevin-lr-init", type=float, default=0.1)
    p.add_argument("--langevin-lr-final", type=float, default=1e-5)
    p.add_argument("--dfo-noise-init", type=float, default=None,
                   help="DFO jitter std (IBC's iteration_std; default from the "
                        "checkpoint, 0.33).")
    p.add_argument("--dfo-noise-decay", type=float, default=None,
                   help="per-iteration jitter decay (IBC's std_decay; default "
                        "from the checkpoint, 0.5).")
...
# ---------------------------------------------------------------------------
# Observation handling (mirrors eval_widowx_bfn.py, then ibc preprocessing)
# ---------------------------------------------------------------------------
...
# and the same mapping in scripts/deploy_pusht_real.py). There is
...
# ---------------------------------------------------------------------------
# IBC policy (the ONLY block that differs from deploy_pusht_real.py)
# ---------------------------------------------------------------------------
# Where the Q3C client builds a control-point generator that PROPOSES ~20
# candidate actions plus a Q net that ranks them, IBC has a single energy model
# (PixelEBM = conv/resnet encoder + DenseResnetValue == utils.models
# .PixelQEstimator) and no proposal network at all: it searches a large uniform
# action cloud with derivative-free optimization -- 2048 samples x 3 rounds of
# score -> softmax resample -> shrinking Gaussian jitter -> argmax, which is
# google-research/ibc's configs/pushing_pixels/pixel_ebm_best.gin and what
# scripts/train_pusht_real_ibc.py says its checkpoints are served with.
#
# The search itself is utils.ibc_policy.dfo_select, imported rather than
# reimplemented so this client and the offline diagnostic
# (scripts/diagnose_pusht_actions_ibc.py) cannot drift apart. Model
# construction is done HERE, not by utils.ibc_policy.load_policy, for the same
# reason build_models exists in the Q3C client: the encoder knobs (resnet18 /
# gn / num_kp 128 -- see batches/pushtWidowXlibIbc*.txt) must be forwarded or
...
def build_model(env: dict, in_channels: int, device, cond_dim: int = 0,
                norm_stats: dict | None = None):
    """Reconstruct the PixelEBM exactly as train_pusht_real_ibc.py built it.

    Same contract as the Q3C client's `build_models`, minus the control-point
    generator IBC does not have: every knob the trainer read out of the run
    config's `model` block is forwarded here, because the ResNet-18 lines
    (batches/pushtWidowXlibIbc*.txt: --encoder-kind resnet18 --encoder-norm-kind
    gn --encoder-num-kp 128) differ from the conv_maxpool defaults in the
    ...
    fixes the action width; the config's `action_dim` stays 2 even for a
    chunked run.
    """
...
def resolve_checkpoint(seed_dir: Path, ckpt_step: int) -> Path:
    """Which weights to load. IBC saves no EMA copy, so there is no suffix.

    An explicit --ckpt-step wins; otherwise the final q_estimator.pt, falling
    back to the newest q_estimator_step*.pt so a seed that is still training is
    still deployable rather than failing outright.
    """
    if ckpt_step > 0:
    ...
    print(f"[WARN] no final q_estimator.pt in {seed_dir}; using snapshot "
          f"{snapshots[-1].name} (training likely still in progress)")
    return snapshots[-1]


def dfo_settings(env: dict, args) -> Dict[str, Any]:
    """The DFO search parameters, config first, then explicit CLI overrides.

    Defaults are pixel_ebm_best.gin's (2048 x 3, std 0.33 halving, boundary
    buffer 0.05) -- the policy train_pusht_real_ibc.py documents its checkpoints
    as being served with. A flag only takes effect when it was actually passed,
    so a run that recorded its own numbers keeps them.
    """
    from utils.ibc_policy import DFO_DEFAULTS
    ...
    dfo = {
        ...
        "iteration_std": float(inf.get("dfo_iteration_std",
                                       DFO_DEFAULTS["iteration_std"])),
        "std_decay": float(inf.get("dfo_std_decay", DFO_DEFAULTS["std_decay"])),
        "boundary_buffer": float(inf.get("uniform_boundary_buffer",
                                         DFO_DEFAULTS["boundary_buffer"])),
    }
    if args.dfo_samples is not None:
        dfo["samples"] = int(args.dfo_samples)
    if args.dfo_noise_init is not None:
        dfo["iteration_std"] = float(args.dfo_noise_init)
    if args.dfo_noise_decay is not None:
        dfo["std_decay"] = float(args.dfo_noise_decay)
    # --refine-iters is the DFO round count only in dfo mode; in langevin mode
    # it is the MCMC step count and must not be written into the DFO settings,
    # which also feed nfe_info().
    if args.refine_iters is not None and args.inference == "dfo":
        dfo["iterations"] = int(args.refine_iters)
    return dfo


def load_policy(seed_dir: Path, env: dict, norm_stats: dict, in_channels: int,
                cond_dim: int, device, args):
    """Seed directory -> a utils.ibc_policy.IBCPolicy ready to act with.

    Returning that dataclass (rather than a bare model) is what lets
    utils.ibc_policy.dfo_select, .describe() and .nfe_info() be shared verbatim
    with the offline diagnostic.
    """
    from utils.ibc_policy import IBCPolicy

    ebm = build_model(env, in_channels, device, cond_dim=cond_dim,
                      norm_stats=norm_stats)
    ckpt_path = resolve_checkpoint(seed_dir, int(args.ckpt_step))
    load_weights(ebm, ckpt_path, device)
    ...
    return IBCPolicy(
        ...
        camera_streams=list(norm_stats.get("camera_streams",
                                           env.get("camera_streams", ["images1"]))),
        image_hw=(int(env.get("image_height", 240)), int(env.get("image_width", 320))),
        action_bounds=(float(a_lo), float(a_hi)),
        dfo=dfo_settings(env, args),
    )


def uniform_cloud(policy, device) -> torch.Tensor:
    """(1, N, A) uniform samples in the action box widened by the DFO buffer.

    IBC has no proposal network, so this cloud is what every selection mode
    starts from -- it is the counterpart of Q3C's control points. The buffer
    matches mcmc.iterative_dfo, which deliberately admits candidates slightly
    outside the trained range; callers clamp before anything reaches hardware.
    """
    a_lo, a_hi = policy.action_bounds
    buf = (a_hi - a_lo) * float(policy.dfo["boundary_buffer"])
    n = int(policy.dfo["samples"])
    return torch.empty(1, n, policy.ebm.action_dim, device=device).uniform_(
        a_lo - buf, a_hi + buf)


def _refine_langevin(ebm, features, init_samples, amin, amax, iters, lr0, lr1):
    """Langevin MCMC refinement over the same energy (energy = -ebm.score).

    The gradient-based sampler from the IBC paper, offered as an alternative to
    DFO on the identical energy model, through the trainer's own
    utils.sampling.sample_langevin so the schedule matches.
    """
    from utils.sampling import sample_langevin

    def energy_fn(_obs, actions):
        return -ebm.score(features, actions).squeeze(-1)

    refined = sample_langevin(
        energy_function=energy_fn, observations=features,
        num_samples=init_samples.shape[1], action_min=amin, action_max=amax,
        num_iterations=iters, lr_init=lr0, lr_final=lr1,
        initial_actions=init_samples,
    )                                                 # (1, N, A)
    with torch.no_grad():
        scores = ebm.score(features, refined).squeeze(-1)
        best = int(scores.squeeze(0).argmax().item())
    return refined[0, best]


def select_action(policy, obs_u8, cond: "torch.Tensor | None" = None,
                  inference: str = "dfo", refine_iters: int = 3,
                  langevin_lr=(0.1, 1e-5)):
    """Pick an action from the energy model.

    `inference`:
      - "dfo": IBC's iterative_dfo (default), i.e. TRAINING inference -- the
        policy pixel_ebm_best.gin pairs with these weights. Shared verbatim
        with the diagnostic via utils.ibc_policy.dfo_select.
      - "argmax"/"sample": rank the initial uniform cloud once, no refinement.
        Diagnostic only: it reads the raw energy surface, which is exactly what
        DFO exists to sharpen, so it is the IBC analogue of the Q3C client's
        pure CP-cloud ranking.
      - "langevin": refine that cloud with Langevin MCMC instead (gradients,
        no resampling).
    `cond` is the (1, cond_dim) conditioning vector; the EBM reads it off
    `_cond`, exactly as the Q3C client feeds its two nets.
    """
    from utils.ibc_policy import dfo_select

    ebm = policy.ebm
    ebm._cond = cond
    if inference == "dfo":
        return dfo_select(policy, obs_u8)[0].detach().cpu().numpy()

    a_lo, a_hi = policy.action_bounds
    features = ebm.encode(obs_u8)                     # (1, feat)
    samples = uniform_cloud(policy, obs_u8.device)    # (1, N, action_dim)
    if inference == "langevin":
        A = samples.shape[-1]
        amin = torch.full((A,), a_lo, device=samples.device)
        amax = torch.full((A,), a_hi, device=samples.device)
        act = _refine_langevin(ebm, features, samples.clamp(a_lo, a_hi),
                               amin, amax, refine_iters,
                               langevin_lr[0], langevin_lr[1])
        return act.detach().cpu().numpy()
    with torch.no_grad():
        logits = ebm.score(features, samples).squeeze(-1)   # (1, N)
        if inference == "sample":
            probs = torch.softmax(logits.squeeze(0), dim=-1)
            idx = int(torch.multinomial(probs, 1).item())
        else:
            idx = int(logits.squeeze(0).argmax().item())
    # Clamp: the boundary buffer admits candidates outside the trained range.
    return samples[0, idx].clamp(a_lo, a_hi).detach().cpu().numpy()
...
def main() -> int:
    ...
    policy = load_policy(seed_dir, env_cfg, norm_stats, in_channels, cond_dim,
                         device, args)
    # Resolve the refinement count ONCE: unset means the checkpoint's own DFO
    # round count, or the trainer's 50 for Langevin. The resolved number is what
    # --measure writes, so the results table never records "default".
    if args.refine_iters is None:
        args.refine_iters = (int(policy.dfo["iterations"])
                             if args.inference == "dfo" else 50)
    print(f"Loaded weights from {policy.checkpoint} (IBC saves no EMA copy)")
    print(f"  frame_stack={frame_stack} cameras={cams} model_hw=({image_h},{image_w}) "
          f"in_channels={in_channels}")
    print(f"  {policy.describe()}  device={device}")
    print(f"  inference={args.inference} refine_iters={args.refine_iters}  "
          f"[NFE] {policy.nfe_info()['details']}")
    ...
    # --- move to the demo start pose (same as the q3c deploy) ---------------
    start_T = None
    ...
    def predict(obs_u8, raw_obs):
        return select_action(
            policy, obs_u8, cond=make_cond(raw_obs),
            inference=args.inference,
            refine_iters=args.refine_iters,
            langevin_lr=(args.langevin_lr_init, args.langevin_lr_final))

...
try:
    ...
    six = None
try:
    import tensorflow as tf  # noqa: F401  # only used inside save/load paths we don't call
except ImportError:
    tf = None
...
def load_urdf(pybullet_client, file_path, *args, **kwargs):
  ...
  try:
    # Vendored layout: rewrite IBC-internal third_party paths to point at this
    # package's assets/ directory (block.urdf, workspace.urdf, suction/*, ...).
    # pybullet ships xarm6_robot.urdf in its pybullet_data directory.
    from .. import ASSETS_DIR  # type: ignore
    if file_path.startswith('third_party/py/ibc/environments/assets/'):
      rel = file_path[len('third_party/py/ibc/environments/assets/'):]
      file_path = str(ASSETS_DIR / rel)
      ...
      file_path = file_path[len(
          'third_party/bullet/examples/pybullet/gym/pybullet_data/'):]
    elif file_path.startswith('third_party/robotics/'):
      # IBC's white-xarm path. Fall back to default xarm6_robot.urdf shipped
      # with pybullet — the white skin is purely cosmetic.
      pybullet_client.setAdditionalSearchPath(pybullet_data.getDataPath())
      file_path = 'xarm/xarm6_robot.urdf'

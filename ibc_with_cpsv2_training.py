"""IBC + Control Point Generator with estimator-guided InfoNCE.

Variation where:
  - The QEstimator is trained with InfoNCE + gradient penalty against Langevin
    MCMC negatives ONLY (no generator control points as counter-examples).
    Paper-faithful hyperparameters (exponential LR decay, action normalization).
...
# ─── Load config ─────────────────────────────────────────────────────────────
# When driven by hyperparam_search.py, Q3C_CONFIG_PATH points at a per-trial
# config file — that's how parallel hyperparam trials avoid racing on the
# shared config.json. Falls back to the default path for standalone runs.
config_path = Path(
...
# Target estimator sync interval (for generator InfoNCE stability)
TARGET_UPDATE_INTERVAL = env_training.get(
    "target_update_interval",
    training_shared.get("target_update_interval", 1000),
)
...
def main():
    ...
    # Target estimator: frozen snapshot, periodically synced. Used for the
    # generator's InfoNCE so the generator optimizes against a stable Q.
    target_estimator = deepcopy(estimator).to(device)
    for p in target_estimator.parameters():
        p.requires_grad_(False)
    ...
    while step < training_steps:
        for batch in dataloader:
            ...
            # ============================================================
            # Langevin hard-negatives (shared by both losses). Generated
            # against the LIVE estimator so the estimator still sees hard
            # negatives for its own update; the generator reuses the same
            # set with the target estimator below.
            # ============================================================
            ...
            # hard-negatives as counter-examples (target estimator, gradient
            # flows only to the generator) + Separation (+ optional MSE).
            # ============================================================
            predicted_actions = control_point_generator(states_norm)

            # Normalize CPs into the estimator's training space ([0,1]).
            # Gradient flows through cp_norm back into the generator.
            cp_norm = (predicted_actions - act_min_t) / act_rng_t  # (B, N_cp, A)

            # Step 1 (no grad): score all CPs with target estimator, pick top.
            with torch.no_grad():
                obs_expanded_cp = states_norm.unsqueeze(1).expand(-1, control_points, -1)
            ...
            logits_gen = q_gen / SOFTMAX_TEMPERATURE
            log_probs_gen = logits_gen - torch.logsumexp(logits_gen, dim=1, keepdim=True)
            ...
            # Periodic target estimator sync (hard update) for generator stability
            if step % TARGET_UPDATE_INTERVAL == 0:
                target_estimator.load_state_dict(estimator.state_dict())
                for p in target_estimator.parameters():
                    p.requires_grad_(False)
            ...
            if step % log_interval == 0:
                ...
                print(
                    ...
                    f"(GenNCE: {loss_gen_infonce.item():.4f}, "
                ...
                wandb.log({
                    ...
                    "loss/generator_infonce": loss_gen_infonce.item(),

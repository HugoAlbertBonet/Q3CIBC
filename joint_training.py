"""Joint training script: MSE-grounded generator + InfoNCE estimator with shared gradients.

Key differences from combined_training.py:
  1. Generator is NOT detached — InfoNCE gradient flows back to generator
  2. Expert action is directly Q-evaluated (not just interpolated via wire fitting)
  3. Uses wireFittingNorm with all points (expert + control points) directly evaluated
  4. Generator loss = MSE + Separation + alpha * InfoNCE (joint signal)
...
# Joint training weight for InfoNCE contribution to generator loss
infonce_weight = training_shared.get("infonce_weight", 1.0)
...
def main():
    ...
    print(f"InfoNCE weight (alpha): {infonce_weight}")
    ...
    wandb.init(
        ...
        name=f"{active_env}_joint_cp{control_points}_lr{learning_rate}"
    )
    ...
    # Smoothing parameter — wireFittingNorm expects (B, N+1) shape
    ...
    while step < training_steps:
        for batch in dataloader:
            ...
            # Evaluate expert action directly
            expert_estimation = estimator(states, actions).squeeze(-1)  # (B,)
            ...
            # Evaluate counter-examples directly
            states_counter = states.unsqueeze(1).expand(-1, num_counter_examples, -1)
            counter_estimations = estimator(states_counter, counter_samples).squeeze(-1)  # (B, M)
            
            # Wire fitting normalization: expert + control points + counter-examples
            # Combine all points and values for wire fitting
            all_control_points = torch.cat([predicted_actions, counter_samples], dim=1)  # (B, N+M, D)
            all_control_values = torch.cat([estimations, counter_estimations], dim=1)    # (B, N+M)
            
            # wireFittingNorm returns (B, N+M+1), first column = expert
            estimations_all = wireFittingNorm(
                control_points=all_control_points,
                expert_action=actions,
                control_point_values=all_control_values,
                expert_action_value=expert_estimation,
                c=smoothing_param_tensor.expand(states.shape[0], all_control_points.shape[1] + 1),
            )
            
            loss_infonce = lossInfoNCE(estimations_all)
            ...
            # ==================== Joint update ====================
            # Generator gets MSE + Separation + weighted InfoNCE (gradients flow through control points)
            loss_generator = loss_mse + loss_sep + infonce_weight * loss_infonce
            # Estimator gets InfoNCE only
            loss_estimator = loss_infonce
            
            optimizer_generator.zero_grad()
            optimizer_estimator.zero_grad()
            # Single backward pass — gradients flow to both generator and estimator
            (loss_generator + loss_estimator).backward()
            ...
            if step % log_interval == 0:
                ...
                print(f"Step {step}/{training_steps} | Loss Gen: {loss_generator.item():.4f}, "
                      f"Loss Est: {loss_est_per_sample:.4f}, Loss MSE: {loss_mse.item():.4f}, "
                      f"Loss Sep: {loss_sep.item():.4f} | LR: {learning_rate:.2e} | Elapsed: {elapsed:.1f}s")
                
                log_dict = {
                    "step": step,
                    "loss/generator": loss_generator.item() / batch_size_actual,
                    "loss/estimator": loss_est_per_sample,
                    "loss/infonce": loss_infonce.item() / batch_size_actual,

"""Direct training script (MSE generator + direct InfoNCE estimator).
...
# Langevin config (only used if sampling_method == "langevin")
...
def main():
    ...
    wandb.init(
        ...
        name=f"{active_env}_direct_cp{control_points}_lr{learning_rate}_sep{separation_weight}"
    )
    ...
    while step < training_steps:
        for batch in dataloader:
            ...
            # Generate counter-example samples using selected method
            if sampling_method == "langevin":
                # Use Langevin MCMC sampling with energy function from estimator
                ...
                def energy_fn(obs, act):
                    return -estimator(obs, act).squeeze(-1)  # Negative Q-value as energy
                ...
                counter_samples = sample_uniform(
                    num_counter_examples, states.shape[0],
            ...
            # Directly estimate Q-values for expert action and counter-examples
            # Expert action estimation: (B, 1)
            expert_estimation = estimator(states, actions)  # (B, 1)
            
            # Counter-example estimation: expand states for batch processing
            # states: (B, state_dim) -> (B, M, state_dim)
            states_expanded = states.unsqueeze(1).expand(-1, num_counter_examples, -1)
            counter_estimations = estimator(states_expanded, counter_samples).squeeze(-1)  # (B, M)
            
            # Combine: expert first, then counter-examples -> (B, M+1)
            all_estimations = torch.cat([expert_estimation, counter_estimations], dim=1)
            
            # InfoNCE loss: expert action (index 0) should have highest Q-value
            loss_estimator = lossInfoNCE(all_estimations)
